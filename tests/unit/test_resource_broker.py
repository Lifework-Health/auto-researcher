...
def request(
    *,
    ...
    return ResourceRequest(
        request_id="candidate-boundary-17",
        requirements=(
            ResourceRequirement(
                resource_type="cpu",
                quantity=4,
...
def broker_for(
    provider: SequenceProvider, clock: FakeClock, **kwargs
) -> ResourceBroker:
...
def test_opportunistic_admission_requires_continuously_idle_stability_window() -> None:
    ...
    admission = broker_for(provider, clock).wait_for_admission(
        request(
            ...
            stable_idle_seconds=30,
        )
    )

    assert admission.telemetry.wait_seconds == 30
    assert admission.telemetry.stable_idle_seconds == 30
...
def test_foreign_owner_prevents_opportunistic_admission_until_it_leaves() -> None:
    provider = SequenceProvider(
        [
            (cpu_candidate(foreign_owners=("other-worker",)),),
            (cpu_candidate(),),
        ]
    )
    clock = FakeClock()
...
def test_each_candidate_boundary_rechecks_current_resource_state() -> None:
    ...
    second = broker.wait_for_admission(
        request().model_copy(update={"request_id": "candidate-boundary-18"})
    )
...
def test_atomic_lease_prevents_duplicate_worker_claim() -> None:
    clock = FakeClock()
    store = InMemoryResourceLeaseStore()
    candidate = cpu_candidate()
    store.acquire(
