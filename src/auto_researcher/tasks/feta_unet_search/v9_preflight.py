...
def _validate_bound_evidence(raw: dict[str, Any]) -> tuple[str, ...]:
    ...
    if not isinstance(sources, dict) or not all(_hash(item) for item in sources.values()):
    ...
    if (
        ...
        or design.get("protected_finalisation_hours")
        != V9_EXPECTED_FINALISATION_HOURS
    ...
    if raw.get("launch_ready") is not False or not isinstance(blockers, list) or not blockers:
...
def _validate_template(raw: dict[str, Any]) -> tuple[FeTAUNetSearchConfiguration, ...]:
    ...
    if not isinstance(roots, list) or len(roots) != sum(V9_EXPECTED_ROOT_COUNTS.values()):
        raise ValueError("feta_unet_v9_root_portfolio_invalid")
    validated = tuple(FeTAUNetSearchConfiguration.model_validate(item) for item in roots)
    counts: dict[str, int] = {}
