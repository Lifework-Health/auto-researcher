"""Durable, identity-bound FeTA fold result reuse."""
...
FOLD_RESULT_SCHEMA_VERSION = "feta-fold-result-v1"
FOLD_SCIENTIFIC_IDENTITY_VERSION = "feta-segresnet-fold-science-v1"
...
def _identity(
    ...
    """Scientific identity that must match before a fold may be reused."""
...
def persist_fold_result(
    ...
    """Atomically persist one completed fold after its OOF metrics are complete."""
...
def load_fold_result(
    ...
    """Verify and import a previously completed compatible fold."""
    ...
    if payload.get("schema_version") != FOLD_RESULT_SCHEMA_VERSION:
        raise ValueError("feta_fold_resume_schema_mismatch")
    ...
    if payload.get("identity") != expected_identity:
        raise ValueError("feta_fold_resume_identity_mismatch")

    result_payload = payload.get("result")
    if not isinstance(result_payload, dict):
        raise ValueError("feta_fold_resume_payload_invalid")
    ...
    if observed_subjects != expected_subjects:
        raise ValueError("feta_fold_resume_oof_membership_invalid")
    ...
    if not isinstance(checkpoint, dict):
        raise ValueError("feta_fold_resume_checkpoint_invalid")
    ...
    if (
        ...
        raise ValueError("feta_fold_resume_checkpoint_identity_mismatch")
    ...
    new_reference = checkpoint_reference(
        target_checkpoint,
        fold=fold,
    ...
    result_payload["checkpoint"] = new_reference
    result_payload["subject_metrics"] = tuple(result_payload["subject_metrics"])
