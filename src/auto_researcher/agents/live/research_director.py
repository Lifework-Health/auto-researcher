"""Bounded live Research Director decision and deterministic reconciliation."""
...
from auto_researcher.agents.reconciliation import ResearchDirectorReconciler
...
class LiveResearchDirectorAgent:
    def __init__(
        self,
        *,
        ...
        self._prompt = load_prompt("research_director", call_config.prompt_version)
        self._reconciler = ResearchDirectorReconciler()
        self._telemetry: AgentCallTelemetry | None = None

    def decide(self, context: ResearchDirectorContext) -> ResearchDirective:
        try:
            directive, telemetry = self._call.run(
                ...
                prompt=self._prompt,
                response_model=ResearchDirectiveProposal,
